...
const API = {
  analyze: (url: string) =>
    getJson<{
      ...
      isPlaylist?: boolean;
    }>(`/api/analyze?url=${encodeURIComponent(url)}`, { method: "POST" }),
  beers: (
    country: string,
    level?: number,
  ...
  leaderboard: () => getJson<LeaderboardEntry[]>("/api/leaderboard"),
  pairingTried: (body: { trackOrPlaylist?: string; drinkName?: string }) =>
    getJson<{ points: number; level: string }>("/api/pairing-tried", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    }),
  history: () => getJson<ConnectionEntry[]>("/api/history"),
};
...
type LeaderboardEntry = { userId: string; points: number; level: string };
